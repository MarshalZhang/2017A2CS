...
def push(List,StartPointer, FreeListPtr,NewItem):
    List[FreeListPtr].Data=NewItem
    List[FreeListPtr].Pointer=NULLPOINTER
    FreeListPtr=FreeListPtr+1
    StartPointer=StartPointer+1

        
def InsertNode(List, StartPointer, FreeListPtr,NewItem):
    if FreeListPtr != NULLPOINTER :# there is space in the array # take node from free list and store data item
...
def FindNode(List, StartPointer, DataItem) : # returns pointer to node
    ...
    while CurrentNodePtr != NULLPOINTER and List[CurrentNodePtr].Data != DataItem :# not end of list,item not found# follow the pointer to the next node
